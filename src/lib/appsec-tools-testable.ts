/**
 * Vitest-compatible AppSec tools module.
 *
 * The source of truth is supabase/functions/_shared/appsec-tools.ts (Deno context).
 ...
 *
 * Exports: APPSEC_SYSTEM_PROMPT, APPSEC_TOOL_NAMES, parseDependencyManifest,
 *          CONFIG_SECURITY_RULES, reviewConfigFile, buildAppSecPrompt
 *
 * Keep in sync with: supabase/functions/_shared/appsec-tools.ts
 */
import { z } from 'zod';

// ---------- System Prompt (identical to appsec-tools.ts) ----------
...
// ---------- Pure Functions (identical to appsec-tools.ts) ----------
...
// ---------- CONFIG_SECURITY_RULES (identical to appsec-tools.ts) ----------
...
export const CONFIG_SECURITY_RULES: Array<{
  ...
  },
  {
    ...
    severity: 'high',
    description: 'Insecure HTTP protocol detected for service endpoint (should use HTTPS)',
    fix_suggestion:
...
/**
 * Creates 5 AI SDK tool definitions for the AppSec agent.
 * Returns plain objects with description and parameters properties
 * matching the AI SDK tool() shape for testability.
 */
export function createAppSecTools(supabase: any, task: any) {
  return {
    parseDependencyManifest: {
      description:
        'Parse a dependency manifest file (package.json, requirements.txt, or pom.xml) and extract package-version pairs.',
      parameters: z.object({
    ...
    matchDependencyVulnerabilities: {
      description:
        'Cross-reference package names against the threat_intelligence table to find known CVEs.',
      parameters: z.object({
        packages: z
    ...
    reviewConfigFile: {
      description:
        'Review a configuration file for security misconfigurations using CONFIG_SECURITY_RULES checklist.',
      parameters: z.object({
        config_content: z.string().describe('The raw content of the configuration file'),
        config_type: z.string().describe('The file name or type of the configuration file'),
      }),
    },

    createAppSecFinding: {
      description:
        'Create a new AppSec finding in the database with severity, fix suggestion, and CMMC control mapping.',
      parameters: z.object({
        finding_type: z.enum([
        ...
        affected_component: z.string().describe('Affected package, file, or component'),
...
// ---------- Prompt Builder (identical to appsec-tools.ts) ----------
