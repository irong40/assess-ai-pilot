/**
 * SOC Analyst Agent Edge Function
 *
 * Performs CVE alert triage, cross-source correlation, false positive
 * classification, and incident escalation for CMMC-focused defense contractors.
 * All analysis is enriched with the company's tech stack context.
 *
 * Actions:
 * - triage-alerts: Batch triage of recent CVEs against company tech stack
 * - correlate-findings: Cross-source correlation (CVE + gaps + alerts)
 * - classify-alert: Individual alert classification with 4-step reasoning
 *
...
import {
  ...
  createSocTools,
  buildSocPrompt,
} from "../_shared/soc-tools.ts";
import { SocTriageResultSchema } from "../_shared/soc-schemas.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const result = await executeAgentTask(supabase, task, async (t) => {
      ...
      const tools = createSocTools(supabase, t);

      // Use Vercel AI SDK generateText with Claude for SOC triage reasoning
      ...
      try {
        ...
        parsedResult = SocTriageResultSchema.parse(parsed);
      } catch {
        // If structured output parsing fails, return the raw text as output
        // The SOC agent persists alerts via createSocAlert tool during execution,
        // so data is saved even if the final structured parse fails
        return {
          output: {
            raw_response: text,
            parse_error:
              "Failed to parse structured SocTriageResult from LLM output",
            action: t.action,
    ...
    console.error("SOC Analyst agent error:", error);
    return new Response(
      JSON.stringify({
