/**
 * Incident Response Agent Edge Function
 *
 * Consumes SOC-escalated incidents and produces containment recommendations,
 * step-by-step playbooks, and post-incident reports following the NIST SP
 * 800-61r2 four-phase incident response lifecycle.
 *
 * ALL IR tasks use risk_level='high' -- every recommendation requires human
 * approval before any action is taken.
 *
 * Actions:
 * - analyze-incident: Analyze escalated SOC alerts, create IR incident, generate containment plan
 * - generate-playbook: Generate step-by-step playbook for existing incident
 * - create-post-incident-report: Generate post-incident report and update compliance snapshot
 *
...
import {
  ...
  createIrTools,
  buildIrPrompt,
} from "../_shared/ir-tools.ts";
import { IrAnalysisResultSchema } from "../_shared/ir-schemas.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Verify task is high-risk (IR hard constraint)
    if (taskRow.risk_level !== "high") {
      console.warn(
        `IR task ${task_id} has risk_level=${taskRow.risk_level}, expected 'high'. Proceeding but logging warning.`
      );
    }
    ...
    const result = await executeAgentTask(supabase, task, async (t) => {
      ...
      const tools = createIrTools(supabase, t);

      // Use Vercel AI SDK generateText with Claude for IR reasoning
      const { text, steps } = await generateText({
        model: anthropic("claude-sonnet-4-20250514"),
        system: IR_SYSTEM_PROMPT,
        prompt: buildIrPrompt(t.action, t.input ?? {}),
        tools,
      ...
      try {
        ...
        parsedResult = IrAnalysisResultSchema.parse(parsed);
      } catch {
        // If structured output parsing fails, return the raw text as output
        // The IR agent persists data via tools during execution,
        // so incident records and plans are saved even if the final structured parse fails
        return {
          output: {
            raw_response: text,
            parse_error:
              "Failed to parse structured IrAnalysisResult from LLM output",
            action: t.action,
    ...
    console.error("Incident Response agent error:", error);
    return new Response(
      JSON.stringify({
