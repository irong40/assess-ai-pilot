/**
 * GRC Analyst domain-specific tools and system prompt.
 *
 * Provides:
 * - GRC_SYSTEM_PROMPT: NIST 800-171A methodology rules for the LLM
 * - createGrcTools(): 6 AI SDK tool definitions for database access
 * - buildPromptForAction(): action-specific prompt construction
 * - storeGrcResult(): persist gap analysis report + compliance snapshot
 *
...
/**
 * GRC Analyst system prompt encoding all 7 NIST 800-171A methodology rules.
 * This is the compliance reasoning backbone -- rules the LLM MUST follow
 * when analyzing controls, generating findings, and recommending remediation.
 */
...
/**
 * The 6 GRC domain-specific tool names for validation and testing.
 */
 ...
 */
export function createGrcTools(supabase: SupabaseClient, task: AgentTask) {
  ...
  return {
    queryControls: tool({
      ...
      execute: async ({ control_id, family_id, cmmc_level }) => {
        let query = supabase
          .from("controls")
          .select("control_id, title, family_id, family_name, cmmc_level, assessment_objectives, sprs_weight");

        if (control_id) query = query.eq("control_id", control_id);
        if (family_id) query = query.eq("family_id", family_id);
        if (cmmc_level) query = query.lte("cmmc_level", cmmc_level);

        const { data, error } = await query.order("control_id");
    ...
    queryAssessmentResponses: tool({
      ...
      execute: async ({ assessment_id, control_id }) => {
        let query = supabase
          .from("assessment_responses")
          .select("*, assessment_questions!inner(control_id, question_text)")
          .eq("assessment_id", assessment_id);

        if (control_id) {
          query = query.eq("assessment_questions.control_id", control_id);
        }
    ...
    queryFindings: tool({
      ...
      execute: async ({ assessment_id, status }) => {
        let query = supabase
        ...
        const { data, error } = await query;
        if (error) return { error: error.message, findings: [] };
    ...
    calculateSprsScore: tool({
      description:
        "Calculate the SPRS (Supplier Performance Risk System) score for the company based on current assessment responses. The score ranges from -203 to 110. Each NOT_MET control deducts its SPRS weight from the maximum of 110.",
      parameters: z.object({
        assessment_id: z.string().uuid().describe("The assessment ID to calculate SPRS score for"),
      }),
      execute: async ({ assessment_id }) => {
        // Fetch controls with SPRS weights
        const { data: controls, error: controlsError } = await supabase
          .from("controls")
          .select("control_id, sprs_weight, cmmc_level")
          .order("control_id");

        if (controlsError) return { error: controlsError.message, score: null };

        // Fetch assessment responses to determine which controls are met
        const { data: responses, error: responsesError } = await supabase
          .from("assessment_responses")
          .select("*, assessment_questions!inner(control_id)")
          .eq("assessment_id", assessment_id);

        if (responsesError) return { error: responsesError.message, score: null };

        // Map responses by control_id for lookup
        const responseMap = new Map<string, string>();
        for (const r of responses ?? []) {
          const controlId = (r as any).assessment_questions?.control_id;
          if (controlId) responseMap.set(controlId, r.status ?? "not_assessed");
        }

        // Calculate SPRS: start at 110, deduct weight for each NOT_MET control
        let score = 110;
        const notMetControls: Array<{ control_id: string; weight: number }> = [];
        for (const ctrl of controls ?? []) {
          const status = responseMap.get(ctrl.control_id) ?? "not_assessed";
          if (status !== "met" && status !== "not_applicable") {
            const weight = ctrl.sprs_weight ?? 1;
            score -= weight;
            notMetControls.push({ control_id: ctrl.control_id, weight });
          }
        }
        ...
        return {
          ...
          assessed_count: responseMap.size,
          not_met_controls: notMetControls,
          poam_eligible: score >= 80,
        };
      },
    }),
    ...
    getControlFamily: tool({
      ...
      execute: async ({ family_id }) => {
        ...
        if (error) return { error: error.message, controls: [] };
        return { family_id, family_name: data?.[0]?.family_name ?? family_id, controls: data ?? [] };
      },
    }),
    ...
    getEvidenceStatus: tool({
      ...
      execute: async ({ assessment_id, control_ids }) => {
        const { data, error } = await supabase
          .from("assessment_findings")
          .select("control_id, evidence_references, status")
          .eq("assessment_id", assessment_id)
          .eq("company_id", companyId)
          .in("control_id", control_ids);

        if (error) return { error: error.message, evidence: [] };

        const evidenceMap = (data ?? []).map((f: any) => ({
          control_id: f.control_id,
          has_evidence: (f.evidence_references ?? []).length > 0,
          evidence_count: (f.evidence_references ?? []).length,
          status: f.status,
        }));
...
/**
 * Builds the user prompt based on the task action and input scope.
 */
...
/**
 * Stores a gap analysis result and creates a compliance snapshot.
 */
export async function storeGrcResult(
  ...
  if (insertError) {
    console.error("Failed to store gap analysis result:", insertError);
    return { error: insertError.message };
  }

  // 2. Create compliance snapshot for time-series tracking
  const { error: snapshotError } = await supabase
    ...
    .insert({
      ...
      cmmc_level: report.cmmc_level,
      critical_controls_met: false, // Will be updated by downstream analysis
  ...
  if (snapshotError) {
    console.error("Failed to create compliance snapshot:", snapshotError);
  }
