/**
 * AppSec Engineer Agent Edge Function
 *
 * Scans dependency manifests (package.json, requirements.txt, pom.xml),
 * reviews configuration files for security misconfigurations, and generates
 * vulnerability findings with fix suggestions mapped to CMMC controls.
 *
 * Actions:
 * - scan-dependencies: Parse manifest, match vulnerabilities, create findings
 * - review-config: Apply CONFIG_SECURITY_RULES to config content, create findings
 * - generate-security-report: Aggregate all findings into SecurityReviewReport
 *
...
import {
  ...
  createAppSecTools,
  buildAppSecPrompt,
} from "../_shared/appsec-tools.ts";
import { SecurityReviewReportSchema } from "../_shared/appsec-schemas.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const result = await executeAgentTask(supabase, task, async (t) => {
      ...
      const tools = createAppSecTools(supabase, t);

      // Use Vercel AI SDK generateText with Claude for AppSec reasoning
      const { text, steps } = await generateText({
        model: anthropic("claude-sonnet-4-20250514"),
        system: APPSEC_SYSTEM_PROMPT,
        prompt: buildAppSecPrompt(t.action, t.input ?? {}),
        tools,
      ...
      try {
        ...
        parsedResult = SecurityReviewReportSchema.parse(parsed);
      } catch {
        // If structured output parsing fails, return the raw text as output
        // The AppSec agent persists findings via tools during execution,
        // so findings are saved even if the final structured parse fails
        return {
          output: {
            raw_response: text,
            parse_error:
              "Failed to parse structured SecurityReviewReport from LLM output",
            action: t.action,
    ...
    console.error("AppSec Engineer agent error:", error);
    return new Response(
      JSON.stringify({
